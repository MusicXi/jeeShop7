...
import com.zlkj.ssm.shop.core.entity.MenuType;
import com.zlkj.ssm.shop.core.entity.User;
import com.zlkj.ssm.shop.manage.dao.AdminMenuDao;
...
@Transactional
@Service
public class AdminMenuServiceImpl extends ServersManager<Menu, AdminMenuDao> implements Services<Menu> {
    @Resource
...
//							System.out.println("del menus:" + menu);
	...
	public List<MenuItem> loadMenus(User u, String pid, String url) {
		...
		List<Menu> menus = dao.selectMenus( param);
		// 创建菜单集合
...
//			Menu entry = it.next();
	...
	private void loadChildrenByPid(MenuItem item, Menu menu, String url, User u) {
		...
		// 加载菜单节点
		List<Menu> data = dao.selectList(param);
	...
	@Override
	public PagerModel selectPageList(Menu menu) {
		return null;
	}
}
